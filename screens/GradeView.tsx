...
export const GradeView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (Number(student.grade) !== Number(id)) setIsAuthorized(false);
  ...
  if (isAuthorized === false) return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 text-center">
      <div className="bg-white p-10 rounded-3xl shadow-2xl max-w-md border border-red-100">
        ...
        <p className="text-slate-500 mb-6">Você só pode acessar os conteúdos da sua série cadastrada.</p>
        <Link to="/" className="block bg-slate-900 text-white p-4 rounded-xl font-bold">Voltar</Link>
      </div>
    </div>
  );
